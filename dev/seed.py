...
def seed_organization(db, user, org_name):
    """Create or get the seed organization. Returns the organization record."""
...
def seed_member(db, user, org):
    """Create or get the member linking user to organization. Returns the member record."""
    ...
    existing = (
        db.query(Member)
        .filter(Member.organization_id == org.id, Member.user_id == user.id)
        .first()
    )
    if existing:
        return existing, False
...
def seed_from_configs(db, org_name, email, password):
    ...
    user, user_created = seed_user(db, email, password)
    user_id = user.id

    org, org_created = seed_organization(db, user, org_name)
    org_id = org.id

    member, member_created = seed_member(db, user, org)
    ...
    stats["org_created"] = "created" if org_created else "already existed"
    stats["member_created"] = "created" if member_created else "already existed"
...
def main():
    ...
    try:
        ...
        print(f"\n✓ Setup complete:")
        print(f"   User:      {stats['user_created']} ({email})")
        print(f"   Org:       {stats['org_created']} ({org_name})")
        print(f"   Member:    {stats['member_created']}")
