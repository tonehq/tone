...
| `ten` | TEN VAD (Apache 2.0), faster speech-to-silence transitions than Silero; telephony audio at 8 kHz is upsampled to the model's 16 kHz | 8 and 16 kHz | `ten-vad` from `requirements.txt` plus the `libc++1` and `libc++abi1` Debian packages the Dockerfile installs: the prebuilt `libten_vad.so` links against LLVM's libc++, and without it the provider is hidden and the worker logs `TEN VAD native library cannot load`. Field: `hop_size` (160 or 256 samples) |
| `aic_quail` | ai-coustics VAD (formerly Quail VAD), licensed; the model downloads from their CDN on first use | 16 kHz | `aic-sdk` from `requirements.txt`, pinned to the version the `tone-pipecat` build declares for its `aic` extra (2.5 for 0.0.76.dev8), and `AIC_SDK_LICENSE` from developers.ai-coustics.com. Field: `model_id`; the default is Pipecat's `DEFAULT_QUAIL_VAD_MODEL_ID`, so it always matches the SDK the image ships: `quail-vad-2.0-xxs-16khz` on the 2.5 SDK (model file v5) and `vad-2.1-xxs-16khz` on the 3.x SDK (model file v7). A mismatched pair fails at build with `missing compatible version` |
