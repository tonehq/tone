...
**Shared components**: `src/components/shared/` holds reusable form/UI pieces (TextInput, CustomButton, Form, CheckboxField, RadioGroupField, CustomLink) and RHF wrapper components (FormTextInput, FormTextAreaField, FormSelectInput, FormCheckboxField, FormRadioGroupField). To understand or use them without reading each file, read **`docs/shared-components.md`** (single reference, lower token usage). When adding or changing a shared component, update that doc.

**Form validation**: Two patterns are used for form validation:

1. **Auth forms** (login, signup, forgot password, reset password) — use `react-hook-form` with `zodResolver` and Zod schemas from `src/schemas/auth.ts`. Each form uses `useForm<SchemaType>` + `FormTextInput` for inline error display. Dependencies: `zod`, `@hookform/resolvers`.
2. **Agent forms** — use `react-hook-form` with inline `rules` prop validation on `FormTextInput` / `FormSelectInput` fields. No Zod schemas.

**Important**: When using `FormTextInput`, do NOT also pass `error` to a parent `FormRow` — `FormTextInput` already renders `fieldState.error.message` as `helperText`, causing duplicate error messages.
