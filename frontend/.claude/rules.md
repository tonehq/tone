...
- **Cursor: pointer for clickables (mandatory).** Every interactive/clickable element must show a pointer cursor; disabled controls show `not-allowed`. A global base rule in `src/app/globals.css` already covers `button`, `[role="button"]`, `a[href]`, `label[for]`, `summary`, custom-select triggers, and common ARIA roles (`option`/`menuitem`/`tab`/`radio`/`switch`), so you rarely need `cursor-pointer` on standard controls. When you make a NON-standard element clickable (a `div`/`span` with `onClick`), prefer a real `<button>`/`CustomButton`; if you must keep the element, add `cursor-pointer` (and a `role` + keyboard handler for a11y). Never leave a clickable element on the default arrow cursor.
...
6. **Build reusable, generic, extensible structures FIRST — for the feature at hand AND future ones.** Do not ship a bespoke one-off and "refactor later". When you build a flow that processes/imports/validates/transforms data or drives a multi-step action, design it as composable, extensible parts from the start: (a) normalize inputs into ONE shared type; (b) put the parse/loop/validate/process pipeline in ONE place every input and destination reuses; (c) make it extensible **by extension** — a new input is a new small class/adapter (ABC/interface/registry entry), a new rule is a new validator added to a composite, never an edit to the core loop; (d) skip only the step you don't need (e.g. call the shared validate/loop directly when data is already structured) rather than re-implementing it; (e) route through the existing shared functions (services, hooks, `lib/api` clients, atoms) instead of duplicating create/assign/schedule/fetch logic. The backend `core/services/contact_ingestion/` framework (source ABC + `RecordParser` common loop + composable `RecordValidator`s → shared destination) is the reference model — mirror that shape on the frontend (shared component/hook/util/service + a small adapter per new case). If nothing reusable exists yet, extract a new generic abstraction rather than writing a throwaway flow.
