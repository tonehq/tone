...
export const webhookConfigSchema = z
  ...
  .superRefine((data, ctx) => {
    // A "path" identifier substitutes its value into a `{param}` placeholder in
    // the URL, so that placeholder must be present.
    data.request_identifiers.forEach((id, i) => {
      if (id.in === 'path' && id.param && !data.endpoint_url.includes(`{${id.param}}`)) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['request_identifiers', i, 'param'],
          message: `Add {${id.param}} to the Endpoint URL where the value should go.`,
        });
      }
    });
  });
