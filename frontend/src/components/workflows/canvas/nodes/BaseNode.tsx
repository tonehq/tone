...
export interface BaseNodeProps {
  ...
  variables?: string[];
  /** when provided, a delete button shows on hover */
  onDelete?: () => void;
...
const BaseNode: React.FC<BaseNodeProps> = ({
  ...
  return (
    <div className="group relative w-[268px]">
      {/* target handle */}
      ...
      {onDelete && (
        <CustomButton
          ...
          title="Delete node"
          onClick={onDelete}
          className="nodrag nopan absolute -right-2.5 -top-2.5 z-20 !h-7 !w-7 rounded-full border border-border bg-card p-0 text-muted-foreground opacity-0 shadow-md transition-opacity hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100"
