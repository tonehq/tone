...
class TestGetAllAgents:
    ...
    def test_service_error(self, mock_service_cls, client_as_member):
        mock_service_cls.return_value.get_all_agents.side_effect = HTTPException(status_code=500, detail="DB error")
        resp = client_as_member.get("/api/v1/agent/get_all_agents")
...
class TestDeleteAgent:
    ...
    @patch("ee.api.v1.agents.AgentService")
    def test_service_error(self, mock_service_cls, client_as_member):
        mock_service_cls.return_value.delete_agent.side_effect = HTTPException(status_code=500, detail="DB error")
        resp = client_as_member.delete("/api/v1/agent/delete_agent", params={"agent_id": 1})
        assert resp.status_code in (500, 422, 400)
...
class TestUpsertAgent:
    ...
    @patch("ee.api.v1.agents.AgentService")
    def test_service_error(self, mock_service_cls, client_as_member):
        mock_service_cls.return_value.upsert_agent.side_effect = HTTPException(status_code=500, detail="DB error")
        resp = client_as_member.post(
            "/api/v1/agent/upsert_agent", json={"name": "Agent"}
        )
        assert resp.status_code in (500, 422, 400)
