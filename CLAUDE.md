...
- **Telephony provider registries** — `core/services/transport/registry.py` (`register_telephony_provider`, one `TelephonyProvider` per media protocol: twilio, telnyx, plivo, vonage, exotel, test), `core/services/call_engines/get_call_engine` (outbound origination: twilio, telnyx, plivo, vonage, sip, websocket; each engine owns `generate_twiml`, `answer_media_type` and `hangup_answer` so `core/api/telephony_routes.py` never branches on the provider) and `core/services/call_termination/get_call_terminator` (runtime hang-up). Credentials come from the org channel through `telephony_credentials.get_provider_credentials`, readiness from `_TRANSPORT_PROBES`, number listing from `ChannelService.list_<provider>_phone_numbers` behind `GET /channel/<provider>_phone_numbers`. Providers whose websocket cannot be sniffed (Vonage) put `provider=<slug>` on the `/ws` query string, which pre-seeds `transport_type` + `call_data`; `from`/`to` on that query string backfill the call data for every provider. Add a provider = one file in each of the three packages + registry lines + credential loader + probe + channel fields; guide: `docs/features/telephony-providers.md`.
...
- **`HOSTED_TTS_PROVIDERS`** (`core/services/pipeline/service_factory.py`) — the ONE registry of TTS providers served through `HostedTTSService` (`core/services/pipeline/hosted_tts_service.py`): any vendor with a JSON request-response speech endpoint (Mistral Voxtral, DeepInfra-hosted Chatterbox Turbo and Qwen3-TTS, fal, Replicate, SiliconFlow, Boson). All wire details live on the model row, never in code: `base_url`, and in `meta_data` the `text_field` / `model_field` / `voice_field` / `audio_field` or `audio_url_field` / `strip_wav_header` / `extra_body` / `extra_headers` keys. A new hosted vendor = one slug in the tuple plus a seed entry.
...
This project is indexed by GitNexus as **curitiba** (20535 symbols, 52674 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
