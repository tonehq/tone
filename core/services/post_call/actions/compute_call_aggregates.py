...
class ComputeCallAggregatesAction(PostCallAction):
    ...
    Runs on the ``call_metrics`` queue (same as
    ``ConsolidateMetricsAction``) so it lands after the raw metrics row is
    already persisted by ``CallLogService.complete_call``.
    """
