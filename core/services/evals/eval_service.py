...
from core.services.evals.eval_summary import (
    _DEEPEVAL_SUMMARY_METRICS,
    _diff_scored_rows,
...
# Run roll-up / diff helpers (and the ``_DEEPEVAL_SUMMARY_METRICS`` tuple the
# query/summary helpers below also read) now live in ``eval_summary`` so the
# aggregation + comparison responsibilities sit apart from this orchestrator.
# Imported back under the same names → no call site changed.
    ...
    def run_eval(
        ...
        *,
        ...
        try:
            ...
            # ``run`` attributes are read post-close here exactly as before
            # (the embedder build already did so); the shared builder assembles
            # the same (embedder, store) pair the eval loop used inline.
            embedder, store = build_retriever_from_run(run, api_key=api_key)
