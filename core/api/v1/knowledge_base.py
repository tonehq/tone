...
async def replace_document_file(
    ...
    # Upload new blob first; only swap DB pointer once the new object is in R2.
    ...
    if upload is None:
        # Defensive: re-create the Upload row if it's missing.
        upload = Upload(
            organization_id=org_id,
            container_name=settings.R2_BUCKET_NAME,
    ...
    # Best-effort delete of the old R2 blob (DB is the source of truth).
...
def delete_document(
    ...
    if file_path:
        try:
            ...
            # Storage delete is best-effort; DB state is the source of truth.
            pass
